...
void LoadSaveLibrary::SaveGameFile(FString SaveFile, UWorld* World)
{
	...
	gameInst->SetPlayerSafeLocation(FVector::ZeroVector); //so it will not be valid until set again

	//Get list of all actors that use USavedActorInterface, and iterate them.
...
void LoadSaveLibrary::OnGameLoadedFixup(UWorld* World)
{
	...
	AMasteringCharacter* Char = nullptr; // if ever more than one, we'll need an array and a map for their inventory

	//iterate these arrays backwards as we will remove objects as we go, can also use iterators, but RemoveAt is simpler.
	for (int i = Actors.Num() - 1; i >= 0; --i)
	{
		...
		for (int j = ActorDatas.Num() - 1; j >= 0; --j)
		{
			...
			//these are actors spawned into the world that we also found in our save data
			if (ActorRecord.MyName == *Actor->GetName())
			{
				...
				if (pawn != nullptr)
				{
					//controller needs the rotation, otherwise could be set when play starts
	...
	//These are actors in the world that are not in our save data, destroy them (ex. an old weaponpickup that was picked up.)
	for (auto Actor : Actors)
	{
		Actor->Destroy();
	}
...
void LoadSaveLibrary::FixupPlayer(UWorld* World, class AMasteringCharacter* Char)
{
	...
	//assuming we found our player character and saved out some inventory, this is where we serialize and fix-up.
	if (Char != nullptr)
	{
		...
		if (gameInst->ShouldPersistInventory())
		{
			...
			checkf(gameInst->GetInventory() != nullptr, TEXT("Game Instance is trying to persist invenotry with no inventory set up!"));
